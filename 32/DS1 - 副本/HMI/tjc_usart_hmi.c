...
void TJCPrintf(const char *str, ...)
{
    ...
    int len_would_write = vsnprintf(buffer, STR_LENGTH + 1, str, arg_ptr);
    va_end(arg_ptr);

    int actual_len_to_send = len_would_write;
    if (len_would_write >= STR_LENGTH + 1) { // 检查是否发生截断
        actual_len_to_send = STR_LENGTH; // 如果截断，只发送缓冲区实际有的长度
    } else if (len_would_write < 0) { // vsnprintf 出错
        return; // 或者其他错误处理
    }

    for (int i = 0; i < actual_len_to_send; i++)
    {
...
void TJCDrawCurve(void)
{
	...
	for ( i = 0; i < sample_num; i++)
            {
                TJCPrintf("add 8,0,%d", (Adc_data[i])*195/4096);  
...
/******** 串口2 中断服务函数 ***********/
//Write received data to ringbuff
...
void TJC_PageControl(void)
{
	  ...
	  {
		  //校验帧头帧尾是否匹配
		  if(usize >= FRAME_LENGTH && u(0) == 0x5A && u(1) == 0x71 && u(3) == 0xff && u(4) == 0xff && u(5) == 0xff)
		  {
				//匹配，进行解析
				if(u(2) == 0x00)// to main page
				{
					//TJCPrintf("page page0");
					CurrentPage = 0;
					
					
				}
				else if(u(2) == 0x01) //to page1
				{
					//TJCPrintf("page page1");
					CurrentPage = 1;
					
					
				}
				else if(u(2) == 0x02) // to page2
				{
					//TJCPrintf("page page2");
					CurrentPage = 2;
				}
				else if(u(2) == 0x03) // to page3
				{
					//TJCPrintf("page page3");
					CurrentPage = 3;
				}
...
void TJC_ChangeTxt(char blank,double Value)
{
	TJCPrintf( "t%s.txt=\"%.3f\"", blank, Value);
}
