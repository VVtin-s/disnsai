...
u32 Ri,Ro,Gain,G_measur=0;							//Ri->输入阻抗; Ro->输出阻抗; Gain->总增益; G_measur->测试仪的增益;
u16 ADD1,ADD2;
u16 text[5];																		  
...
void GPIO_Gate(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10;  // PB8——>A,PB9——>B,PB10——>EN
	...
	// 测试功能
			...
			{			
				...
				if(AD0_data>=600)	
					...
					{	
						Ri=GRS*1.0*(5100*(US-VS))/VS;        									// 正常工作时的输入电阻，根据分压公式计算得到，US为信号源电压
					}  																			
        ...
        change_adc();                             								// 如果AD2值小于100mV, 说明三极管电路放大倍数较小, 32内部ADC测试不准, 做进一步处理				
			...
			if(CurrentPage==3)
			{
				...
				{	
					...
					key_numb=Keypad_Scan();
					if(key_numb == 4)																					// 按键扫描到K4, 则退出故障检测
						break;
				}				
			}
...
void getcaculation(void)
{
        Gate_Set_A;																								// BA=01: 衰减器选择大倍数的, 后端交流检测的增益为1
				Gate_Clr_B;
				Delay_ms(1950);
				AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(1000), 0);//AD9833_DDS产生1KHz正弦信号
				
				// 测量输入端串入的采样电阻两端的电压Vs
				Delay_ms(50);																						// 因为R2断开测电阻时由于RC充放电需要时间，导致波形出现比较慢，所以延时两秒等波形出现再测
...
{		    
        ...
        if(AD0_data>3140)																			//测量实际电范围大概为1K~2K
				...
				if(AD0_data<600)																			//测量实际电范围大概为19K~50K 阻抗大于该值就切换为5.1k取样电阻，AD0_data是ad1采集Rs两端的电压值，根据不一样的电压范围微调放大倍数GRs
					{
							AD1_data=0;     
							Gate_Clr_A;																			// 当flag=1（测基本参数）时，控制增益的模拟开关的B、A置0、1
							Gate_Set_B;																			// BA=01: 衰减器选择大倍数的, 后端交流检测的增益为1
							Delay_ms(200);
							ADC_TIM3_Init(100,1);
							DMA_Cmd(DMA1_Channel1, ENABLE);					 
							ADC_Cmd(ADC1, ENABLE);				 
...
//-----------------------------------------------------------------
	void test(void)
	{
	      // 测试三极管正常放大时, 输入400Hz信号的输出交流电压值
				AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(400), 0); // AD9833-DDS产生400Hz的正弦信号
				AD2=0;
				for(i=0;i<20;i++)
				{
					ADC_Sampling ();												// ADC2批量数据采集并获取峰峰值
				...
				AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(160000), 0);// AD9833-DDS产生160kHz正弦信号
				AD2=0;
				for(i=0;i<20;i++) 
				{
					ADC_Sampling ();												// ADC2批量数据采集并获取峰峰值
				...
				AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(4), 0);	// AD9833-DDS产生4Hz正弦信号				
				vpp_vpp=0;
...
void Sweep_out (u32 fre_l, u32 fre_h)
{
	...
	volatile u16 fre_count,i=0,j=0,k=0;
  ...
  for(i=0;i<fre_count;i++)
	{
		...
		AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(fre_out), 0); // 对应输出1Hz到1MHz的扫频信号
	...
	// 以下下限频率不是题目要求, 这里作为参考
	AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(1000), 0);	// 9833恢复输出1kHz的波形
...
//	Vrs=1.0*Vrs/(495);									    // 495,这是差分放大的倍数, 根据实际测试情况可校准这个系数
	...
	AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(1000), 0);
//*******************************************//
//	测量不准时可以取消下列注释后进行调试修改
//*******************************************//
...
void PoinTFreTest (void)
{
	...
	// 160kHz正弦信号点频测试
	AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(160000), 0);// DDS输出160kHz正弦信号
	AD2=0;
	...
	// 400Hz正弦信号点频测试
	AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(400), 0);	// DDS输出400Hz正弦信号
	AD2=0; 
	for(i=0;i<20;i++)												 // 采集20次
	{
		ADC_Sampling ();
	...
	// 4Hz正弦信号点频测试	
	AD9833_SetFrequency(AD9833_REG_FREQ0, AD9833_OUT_SINUS, freq_trl_word(4), 0);	// DDS输出4Hz正弦信号					
