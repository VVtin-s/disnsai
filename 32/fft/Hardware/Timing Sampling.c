...
static u16 ADC3_ConvertedValueTab[15];
...
void ADC_RegularChannel_Configuration(void)
{
	...
	ADC_RegularChannelConfig(ADC3, ADC_Channel_13, 1,ADC_SampleTime_1Cycles5);		//PC0，采集输出直流
  ...
  for(i = 1; i < sample_num / 2; i++)//由于FFT的频域结果是关于奈奎斯特频率对称的，所以只计算一半
  {
    ...
    if (FFT_Mag[i] > maxMag) 
      ...
      secondMaxIndex = i;
		...
		else if (FFT_Mag[i] > thirdMaxIndex) 
		{
      thirdMaxMag = FFT_Mag[i];
      thirdMaxIndex = i;
    }
	}
  secondMaxIndex=maxIndex;            
...
u16 ADC_MATH(void)
{
	u16 min=65535,max=0,MMM;
	u16 i;

	// 比较得到最大值和最小值	
	for(i=0;i<CodeSize*Channel;i++)		
	{
		if(Adc_data1[i]>max)
		{
			max=Adc_data1[i];
	...
	MMM=max-min;
	vpp_vpp=(MMM*VREF)/4096;					// 数字量转成模拟量
	return vpp_vpp;	
}	
