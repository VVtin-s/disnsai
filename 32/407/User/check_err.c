...
//	以下显示便于直观观察测量值
//*******************************************//
	TJCPrintf( "n1.val=%.3f", DC);
	TJCPrintf( "n3.val=%.3f", ad2);
	TJCPrintf( "n4.val=%.3f", gain);
	TJCPrintf( "n2.val=%.3f", ad1);
...
u8 ERR_c(void)
{
  ...
  TJCPrintf( "n0.val=%.3f", RS_ref1);
	//sprintf((char *)buf,"RS_ref1:%5f, ",);
	TJCPrintf( "n5.val=%.3f", RS_FL1);
