	...
	// 2C1/2C2/2C3/C3开路这四种故障不影响三极管放大倍数, 即和正常工作时输出交流大小基本相等
		if(ad2 >= AC_ref-100 ) 									// 电容故障时测得的输出交流ad2与正常工作时输出交流AC_ref进行比较
		{
			...
			else if(AC_FH < AC_ref2-50) 
			{
				...
				if((RS_FL1 >= RS_ref1-50)&&(RS_FL1 <= RS_ref1+100))	// C1与2倍C1的电压值相差26mvpp左右
				{
...
//		LCD_WriteString(140,120,YELLOW,BLACK,(u8 *)buf);
	Gate_Set;   																	    	// 空载
	//printf("Ri = %u\r\n", Ri);
	if(ri < Ri- 50)     																// 有错时测得的输入电阻ri与正常时的输入电阻Ri比较
		errc = ERR_R2_Short; 
	else 
		if(ri > Ri+600)
		{
			//printf("RS_v = %f\r\n", RS_v);
			//printf("RS_ref = %u\r\n", RS_ref);
			if((int)RS_v < (RS_ref/3.33)- 150)								    // RS_v表示检查故障时串接电阻两端放大后的电压值
			errc = ERR_R1_Open;
			else  if((int)RS_v >= ((RS_ref/3.22)- 150)&&(RS_v<=RS_ref-100)) // 因为RS端电压小于正常工作RS_ref的有R1_Open和R4_Open
			errc = ERR_R4_Open;	
		}
		else  
		{			
